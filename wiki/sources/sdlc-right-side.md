...
The E1 routing-eval suite shipped and was removed the same day (see the log entry): two bugs in
the harness itself, one finding, and a per-run API cost — machinery that cost more than it
returned. [[pre-tool-guard]] — one script, both rules, 24 assertions as `validate.sh` §11, wired in
